...
## Recent Changes
- 2026-02-21: Advisor contact card + time format preferences — website and timeFormat columns on profiles; settings page website input and 12h/24h toggle; formatTime utility (time-utils.ts) and timeFormatString for date-fns; all time displays in trip-view.tsx apply formatting (flights, charters, hotels, restaurants, activities, journeys) with "local" label on flight times; AdvisorContactCard pill-style component replaces plain text footer in trip-view; PDF final page with advisor contact details; PDF time displays use fmtTime; PATCH /api/profile accepts website and timeFormat; getTripFullView returns phone/website/timeFormat for advisor
